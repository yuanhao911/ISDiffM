...
def test_all_batch(args):
    ...
    X_test = simple(X_test, args.simple_interval)
    ...
    model = torch.load(
        './output/pre_diffusion_transfer-learning-from-data1-to-' + args.dataset + "-" + args.mode + '/' + 'pre_diffusion_transfer-learning-from-data1-to-' + args.dataset +'-' + args.mode + '.pt',
        map_location=device)
    ...
    img = torch.randn((len(Y_empty), args.num_proposals, 2), device=device)
    batch = len(Y_empty)
