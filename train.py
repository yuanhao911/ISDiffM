...
def add_noise(signal, snr_db):
    ...
    # Compute noise power
    snr = 10 ** (snr_db / 10)  # Convert SNR in dB to linear scale
...
def setup_seed(seed):
    # Common seeds for numpy and random
    np.random.seed(seed)
    random.seed(seed)

    os.environ['PYTHONHASHSEED'] = str(seed)  # Disable hash randomization
    # For CUDA 10.2 and above, this environment variable is needed to ensure determinism
    ...
    # Use deterministic algorithms to guarantee reproducibility
    torch.use_deterministic_algorithms(True)
    torch.backends.cudnn.deterministic = True  # Ensure deterministic convolution algorithms
    torch.backends.cudnn.enabled = False  # Disable non-deterministic cuDNN algorithms
    # If benchmark=True, cuDNN will search for the best algorithm, which may hurt reproducibility
